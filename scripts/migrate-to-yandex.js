...
const MONGO_URI = process.env.MONGO_MIGRATE_URI || 'mongodb://127.0.0.1:27017/dekorativ_data';
const MONGO_DB = 'dekorativ_data';

// Yandex S3 Configuration
const S3_BUCKET = process.env.YC_S3_BUCKET || 'dekorativ-media';
const S3_ENDPOINT = 'https://storage.yandexcloud.net';
const S3_REGION = 'ru-central1';

// Firebase Storage bucket
const FIREBASE_STORAGE_BUCKET = 'dekorativ-5c737.firebasestorage.app';
...
async function migrateMedia(mongoDb, s3Client) {
    ...
    const portfolioPhotos = await mongoDb.collection('portfolioPhotos').find({}).toArray();
    const fonts = await mongoDb.collection('fonts').find({}).toArray();
    
    let totalUploaded = 0;
    let totalFailed = 0;
    const urlMap = new Map(); // old URL → new URL

    const urls = []; // { collection, id, field, url }

    // 1. Collect from products
    for (const product of products) {
        if (product.images && Array.isArray(product.images)) {
            product.images.forEach(img => {
                if (typeof img === 'string' && img.includes('firebase')) urls.push({ collection: 'products', id: product._id, field: 'image', url: img });
                else if (typeof img === 'object') {
                    if (img.url && img.url.includes('firebase')) urls.push({ collection: 'products', id: product._id, field: 'image.url', url: img.url });
                    if (img.cardUrl && img.cardUrl.includes('firebase')) urls.push({ collection: 'products', id: product._id, field: 'image.cardUrl', url: img.cardUrl });
                    if (img.thumbUrl && img.thumbUrl.includes('firebase')) urls.push({ collection: 'products', id: product._id, field: 'image.thumbUrl', url: img.thumbUrl });
                }
            });
        }
        if (product.videoPreviewUrl && product.videoPreviewUrl.includes('firebase')) urls.push({ collection: 'products', id: product._id, field: 'videoPreviewUrl', url: product.videoPreviewUrl });
        if (product.videoUrl && product.videoUrl.includes('firebase')) urls.push({ collection: 'products', id: product._id, field: 'videoUrl', url: product.videoUrl });
    }

    // 2. Collect from portfolioPhotos
    for (const photo of portfolioPhotos) {
        if (photo.imageUrl && photo.imageUrl.includes('firebase')) urls.push({ collection: 'portfolioPhotos', id: photo._id, field: 'imageUrl', url: photo.imageUrl });
    }

    // 3. Collect from fonts
    for (const font of fonts) {
        if (font.url && font.url.includes('firebase')) urls.push({ collection: 'fonts', id: font._id, field: 'url', url: font.url });
    }

    for (const { collection, id, field, url } of urls) {
        ...
        try {
            ...
            if (pathMatch) {
                ...
                const ext = url.match(/\.(jpg|jpeg|png|webp|mp4|gif|ttf|otf|woff2?)/) ? url.match(/\.(jpg|jpeg|png|webp|mp4|gif|ttf|otf|woff2?)/)[0] : '.bin';
                s3Key = `${collection}/${id}/${field}${ext}`;
            }
            ...
            else if (s3Key.endsWith('.ttf') || s3Key.endsWith('.otf') || s3Key.endsWith('.woff')) contentType = 'font/woff2';
    ...
    if (urlMap.size > 0) {
        ...
        for (const product of products) {
            ...
            if (product.images && Array.isArray(product.images)) {
                update.images = product.images.map(img => {
                    if (typeof img === 'string' && urlMap.has(img)) { needsUpdate = true; return urlMap.get(img); }
            ...
            if (product.videoPreviewUrl && urlMap.has(product.videoPreviewUrl)) { update.videoPreviewUrl = urlMap.get(product.videoPreviewUrl); needsUpdate = true; }
            if (product.videoUrl && urlMap.has(product.videoUrl)) { update.videoUrl = urlMap.get(product.videoUrl); needsUpdate = true; }
            
            if (needsUpdate) await mongoDb.collection('products').updateOne({ _id: product._id }, { $set: update });
        }

        // Update Portfolio Photos
        for (const photo of portfolioPhotos) {
            if (photo.imageUrl && urlMap.has(photo.imageUrl)) {
                await mongoDb.collection('portfolioPhotos').updateOne(
                    { _id: photo._id },
                    { $set: { imageUrl: urlMap.get(photo.imageUrl) } }
                );
            }
        }
        ...
        // Update Fonts
        for (const font of fonts) {
            if (font.url && urlMap.has(font.url)) {
                await mongoDb.collection('fonts').updateOne(
                    { _id: font._id },
                    { $set: { url: urlMap.get(font.url) } }
                );
            }
        }

        ok(`URLs updated in MongoDB across all collections.`);
    }
...
async function main() {
    log(`\n${C.bold}${C.cyan}╔══════════════════════════════════════════════════════════╗`);
    log(`║  🚀  DEKORATIV — FIREBASE → YANDEX MIGRATION SCRIPT    ║`);
    log(`╚══════════════════════════════════════════════════════════╝${C.reset}\n`);
